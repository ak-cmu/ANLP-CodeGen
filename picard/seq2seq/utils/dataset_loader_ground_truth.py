...
def load_dataset(
    ...
    #train = pd.read_pickle("data/train_relevanttables_output.pkl")
    _spider_dataset_dict_akruti: Callable[[], DatasetDict] = lambda: datasets.load_dataset("pandas", data_files={"train":"data/train_relevanttables_output.pkl", "validation":"data/validation_relevanttables_output.pkl"})
    #test = pd.read_pickle("data/train_relevanttables_output.pkl")
    #_spider_dataset_dict_akruti_test: Callable[[], DatasetDict] = lambda: datasets.Dataset.from_pandas(test)
    ...
    if data_args.dataset == "spider":
        ...
        dataset_splits = prepare_splits(
            dataset_dict=_spider_dataset_dict_akruti(),
        ...
        spider_training_split = (
            _prepare_train_split(
                dataset=_spider_dataset_dict_akruti()["train"],
                data_training_args=data_training_args,
                add_serialized_schema=_spider_add_serialized_schema,
                pre_process_function=_spider_pre_process_function,
            )
